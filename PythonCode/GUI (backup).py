...
class File_Reading(QThread):

    def __init__(self,name,sampel_size):
    ...
    def run(self):
        #file_name = "/home/moez/Desktop/data_7_15/wRx_A_1"
        file = open(self.file_name, 'r')

        generator = self.file_generator(file)
...
class Window(QtGui.QMainWindow):
    ...
    def run_event(self):
        ...
        #70 is lower limit
        if truth:
            sample_number = numpy.ceil(self.sample_number_cal(time))

            print ("Selected sampling time interval is %0.1f ms") % time
            print ("Calculated sampling rate is %i") % sample_number

            self.file_reading_thread = QThread()
            self.read = File_Reading(file_name, sample_number)
            ...
            print "No sampling time selected"
